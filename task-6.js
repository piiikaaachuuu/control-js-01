...
// !СПОСІБ №1 (без перевірки чи ввів саме число та за допомогою do..while)
// let input;
// const numbers = [];
// let total = 0;
...
// !СПОСІБ №2 (з перевіркою та за допомогою while та if..else)
let input;
const numbers = [];
let total = 0;
...
while (true) {
  ...
  if (input != null && input != "") {
    if (isNaN(input)) {
      alert("Ви ввели не число, повторіть ще раз!");
    } else {
      numbers.push(Number(input));
    }
  } else break;
...
// !СПОСІБ №3 (без використання масива)
